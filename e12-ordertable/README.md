...
Goal of this assignment is to determine the build *Orders* from *Customers* and *Articles*,
calculate prices and VAT taxes and print the *Order Table* at the end.

*Builder* is another
[*Software Design Pattern*](https://refactoring.guru/design-patterns)
to construct more complex objects in multiple-steps.
...
Class [*OrderBuilder.java*](OrderBuilder.java) implements the *Builder Pattern*
for *Orders*. Main parts are:
    ...
    public final class OrderBuilder {
        ...
        */
        ...
        public final class BuildState {
            Optional<Customer> customer ... ;   // customer looked up from 'customers' collection
            Optional<Order> order = ... ;       // partially built order

            ...
            // methods for build steps:
            BuildState step1_findCustomerBySpec(String customerSpec) { ... }
            BuildState step2_createOrderForCustomer() { ... }
            BuildState step3_attachOrderedItems(Consumer<BuildState> addItems) { ... }
        }
         ...
         * @param pricingCategory {@link PricingCategory} for which order is built.
         ...
         */
        public Optional<Order> buildOrder(PricingCategory pricingCategory,
                String customerSpec, Consumer<BuildState> addItems)
        {
            BuildState buildState = new BuildState(pricingCategory)
...
1. Since orders are no longer created using methods `createOrder(...)`, their
    visibility changes to *protected* and the two-argument version can be
    removed:
    ```java
    /**
    ...
    Optional<Order> createOrder(Optional<Customer> customer, Consumer<Order> orderConsumer) { ... }
    ```

1. Instead, the new method `createOrderBuilder(...)` is introduced in *DataDactory*:
    ```java
    /**
     * Create {@link OrderBuilder} instance from which orders can be built for
     * the specified {@link Pricing.PricingCategory} from provided customers
     * and articles.
     ...
     * @return {@link OrderBuilder} instance to create orders
    ...
    public OrderBuilder createOrderBuilder(PricingCategory pricingCategory,
        Map<Long, Customer> customers, Map<String, Article> articles)
    {
        return new OrderBuilder(pricingCategory, customers, articles);
    }
    ```
...
Create a new driver class `Application_E12.java` in package `application`.
...
Private method: `buildOrders(Map<Long, Order> collector)` builds orders
using the *OrderBuilder*. Objects are collected in the *collector* map
passed as argument.

Consider partial *"order state"* (`ordst -> ordst.addItem(...)`) invoked by
*OrderBuilder* during order construction to add items, only if the order
can be created from valid specs, e.g. name *"Eric"* was found as a customer.
...
*Eric's* first order is built in *Basepricing* (default) for
Germany. Prices are quoted in EUR and German VAT tax rates
apply (*19%* and *7%* reduced rate for books):
...
In category: *"SwissPricing"*, article prices are quoted in CHF and VAT rates
of Switzerland apply (*8.1%* regular and *2.6%* reduced rate).

Although prices are generally higher in Switzerland, VAT tax is lower
(*8.1%* compared to *19%* or *2.6%* compared to *7%* for the reduced rate):
...
1. add class *TableFormatter* from the end of
    [*Application_D12.java*](../d12-datamodel/Application_D12.java) and
...
Supplememt method *buildOrders()* to create the orders shown in the table below.

Run the driver code and print the full OrderTable:
...
Build orders in `SwissPricing` using *CHF* (Swiss Franc) as currency
and Swiss VAT tax rates:
