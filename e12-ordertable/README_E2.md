...
The *datamodel* package has an overall coverage of *84%* :
...
constructor: `Article(String id, String description)` is at *61%*:
...
1. `100:` - `test100_RaiseConstructorExceptionIdNull()` -
...
1. `110:` - `test110_RaiseConstructorExceptionDescriptionNull()` -
    test that an *IllegalArgumentException* is raised when the constructor
...
1. `111:` - `test111_RaiseConstructorExceptionDescriptionEmpty()` -
    test that an *IllegalArgumentException* is raised when the constructor
...
Now, 108 tests should run and pass (3 more over 105 tests above):

```
Test run finished after 542 ms
[       108 tests found           ]
[       108 tests successful      ]
[         0 tests failed          ]
done.
```
```sh
...
Make classes *Customer* and *OrderBuilder* also achieve *100%*:
...
```sh
git commit -m "e2: datamodel, code coverage"
